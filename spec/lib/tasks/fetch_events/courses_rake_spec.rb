...
RSpec.describe 'fetch_events:courses', type: :task do
  ...
  context 'with no events' do
    it 'does not modify any records' do
      expect { subject.invoke }.to  not_change { Course.count }
  ...
  context 'with an exiting course and course events' do
    ...
    context 'update_roster events' do
      ...
      it 'creates or updates CourseContainers and Students for the Course' do
        num_course_containers = num_active_course_containers + num_archived_course_containers
        num_students = num_course_containers * num_students_per_container

        expect { subject.invoke }.to  not_change { Course.count }
                                 .and not_change { EcosystemPreparation.count }
                                 .and change     { CourseContainer.count }.by(num_course_containers)
