...
RSpec.describe Services::FetchCourseEvents::Service, type: :service do
  ...
  context 'with an existing Course and CourseEvents' do
    ...
    context 'update_course_ecosystem events' do
      ...
      context 'with no preexisting ecosystem preparations' do
        ...
        it 'stops right before processing the event' do
          expect(UpdateCourseEcosystemSideEffectsJob).not_to receive(:perform_later)
      ...
      context 'with a preexisting ecosystem preparation' do
        ...
        it "updates the Course's ecosystem_uuid" do
          ...
          expect(UpdateCourseEcosystemSideEffectsJob).to receive(:perform_later).with(
            course_uuids: [ course.uuid ]
          )
    ...
    context 'update_roster events' do
      ...
      let(:num_students) { num_course_containers * num_students_per_container }

      it 'creates or updates CourseContainers and Students for the Course' do
        expect(UpdateRosterSideEffectsJob).to receive(:perform_later).with(
          course_uuids: [ course.uuid ]
        )
    ...
    context 'create_update_assignment events' do
      ...
      let(:assigned_exercise_trial_uuids)     do
        assigned_exercises.map { |ex| ex.fetch(:trial_uuid) }
      end
      let(:assigned_exercise_exercise_uuids)  do
      ...
      context 'with a new Assignment' do
        it 'creates an Assignment for the Course' do
          ...
          expect(assignment.assigned_exercise_uuids).to eq assigned_exercise_exercise_uuids.uniq
