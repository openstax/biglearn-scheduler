    ...
    loop do
      num_calculations = AlgorithmAssignmentSpeCalculation.transaction do
        ...
        algorithm_calculations.size.tap do |num_algorithm_calculations|
          ...
          calculations = AssignmentSpeCalculation.where(uuid: calculation_uuids)
          ...
          # Assignment SPE calculations are partitioned by algorithm, assignment, k_ago and
          # book_container_uuid and combined by algorithm and assignment before sending
          related_calculations = AssignmentSpeCalculation.where(assignment_uuid: assignment_uuids)
          ...
          aaspec = AlgorithmAssignmentSpeCalculation.arel_table
          aaspec_queries = algorithm_calculations.map do |alg_calc|
            related_calcs = related_calcs_by_assignment_uuid[alg_calc.assignment_uuid] || []

            aaspec[:algorithm_name].eq(alg_calc.algorithm_name).and(
              aaspec[:assignment_spe_calculation_uuid].in(related_calcs.map(&:uuid))
            )
          end.compact.reduce(:or)
          related_alg_calcs_by_assignment_uuid_and_alg_name = Hash.new do |hash, key|
            hash[key] = Hash.new { |hash, key| hash[key] = [] }
          end
          related_algorithm_calculations = AlgorithmAssignmentSpeCalculation.where(aaspec_queries)
          ...
          assignment_spe_requests = related_alg_calcs_by_assignment_uuid_and_alg_name
            ...
            related_alg_calcs_by_alg_name.map do |alg_name, related_alg_calcs|
              ...
              exercise_uuids = related_alg_calcs.flat_map do |alg_calc|
                calc_uuid = alg_calc.assignment_spe_calculation_uuid
                calc = related_calcs_by_uuid[calc_uuid]
                if calc.nil?
                  # Something bad happened like a race condition or manual assignment deletion
                  Rails.logger.tagged 'UploadAssignmentSpeCalculations' do |logger|
                    logger.warn do
                      "Assignment SPE skipped due to no information" +
                      " about assignment SPE calculation #{calc_uuid}"
                    end
                  end
