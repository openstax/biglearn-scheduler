class Services::PrepareClueCalculations::Service < Services::ApplicationService
  ...
  def process
    ...
    loop do
      ...
      # once the first one releases the FOR NO KEY UPDATE lock
      ...
      num_responses = Response.transaction(args) do
        ...
        response_query = responses_by_ecosystem_uuid.flat_map do |from_ecosystem_uuid, responses|
          ...
          responses.map do |response|
            ...
            # Find which is the response's book_container_uuid and which ecosystem it should map to
            course_uuid = course_uuid_by_student_uuid[student_uuid]
            to_ecosystem_uuid = latest_ecosystem_uuid_by_course_uuid[course_uuid]
            ...
            # Reverse map to find all book_container_uuids that map to the same book_container_uuid
